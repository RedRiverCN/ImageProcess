...
BOOL CHistogramDlg::OnInitDialog()
{
	...
	m_nPixelCount = wBytesPerLine * wDIBHeight;
	lpDIBBits = m_pDib->GetBitsPtr();

	LONG lOffset;
	for (int i = 0; i < wDIBWidth; i++)
		for (int j = 0; j < wDIBHeight; j++)
		{
			lOffset = PIXEL_OFFSET(i, j, wBytesPerLine);// 计算目标图当前像素偏移量

			m_lCounts[*(lpDIBBits + lOffset)]++;
		}
...
void CHistogramDlg::Refresh()
{
	...
	CString strOrg;
	CStringW strWide = CT2CW(ss); // 将T字符转换成宽字符
	WCHAR *buf = strWide.GetBuffer(3);  // 获取CString内部缓存，并保证缓存长度是STR_LEN

	graph.DrawString(buf, -1, &font,
