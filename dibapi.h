...
//图像处理作业
BOOL NearestInterpolationDIB(HDIB hDib, int Strength);//最近邻插值
BOOL BilinearInterpolationDIB(HDIB hDib);//双线性插值
BOOL BicubicInterpolationDIB(HDIB hDib);//双三次插值
BOOL Mt4DIB(HDIB hDib);//m通路转换为4通路
BOOL ImageNegativesDIB(HDIB hDib);//图像反转
BOOL LogTransformationDIB(HDIB hDib, int iA, int iB);//对数变换
BOOL ContrastStretchingDIB(HDIB hDib, float X1, float Y1, float X2, float Y2);//对比度拉伸
BOOL LocalImageEnhancementDIB(HDIB hDib);////基于直方图统计的局部增强方法
BOOL MyMedianFilterDIB(HDIB hDib);//中值滤波
BOOL SobleOperatorDIB(HDIB hDib);//Soble运算
BOOL LaplacianSharpeningDIB(HDIB hDib);//Laplacian锐化
BOOL PalmprintImageEnhancementDIB(HDIB hDib);//掌纹图像进行图像增强
BOOL FrequencyDomainFilterDIB(HDIB hDib);//等效于3*3邻域均值平滑的频率域滤波
BOOL HomomorphicFilterDIB(HDIB hDib);//同态滤波
BOOL BattivoLowPassFilterDIB(HDIB hDib);//巴特沃思低通
BOOL BattivoHighPassFilterDIB(HDIB hDib);//巴特沃思高通
BOOL BattivoBandpassFilterDIB(HDIB hDib);//巴特沃思带通
BOOL BattivoBandRejectionFilterDIB(HDIB hDib);//巴特沃思带阻
BOOL Exercise443DIB(HDIB hDib);//习题4.43
BOOL VariableThresholdDIB(HDIB hDib);//可变阈值处理
BOOL OstuImageSegmentationDIB(HDIB hDib);//Ostu图像分割方法
BOOL FaceRecognitionDIB(HDIB hDib);//设计人脸方案
BOOL IrisSegmentationDIB(HDIB hDib);//虹膜图像分割
BOOL BoundaryTrackingAlgorithmDIB(HDIB hDib);//边界追踪算法
BOOL RegionRefinementDIB(HDIB hDib);//二值区域细化算法
BOOL GraylevelCooccurrenceMatrixDIB(HDIB hDib);//灰度共生矩阵方法
BOOL Example1116DIB(HDIB hDib);//习题11.16
BOOL Example1127DIB(HDIB hDib);//习题11.27
BOOL FigureRecognitionDIB(HDIB hDib);//编程实现印刷体数字识别
BOOL PeachImageRecognitionDIB(HDIB hDib);//桃子图像识别
