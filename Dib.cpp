...
BOOL CDib::NearestInterpolation(int Strength)
{
	if (IsEmpty())
		return FALSE;

	if (!NearestInterpolationDIB(m_hDib, Strength))
		return FALSE;
...
BOOL CDib::BilinearInterpolation()
{
	if (IsEmpty())
		return FALSE;

	if (!BilinearInterpolationDIB(m_hDib))
		return FALSE;
...
BOOL CDib::BicubicInterpolation()
{
	if (IsEmpty())
		return FALSE;

	if (!BicubicInterpolationDIB(m_hDib))
		return FALSE;
...
BOOL CDib::ImageNegatives()
{
	if (IsEmpty())
		return FALSE;

	if (!ImageNegativesDIB(m_hDib))
		return FALSE;

	return UpdateInternal();
}

BOOL CDib::LogTransformation(int iA, int iB)
{
	if (IsEmpty())
		return FALSE;

	if (!LogTransformationDIB(m_hDib,iA, iB))
		return FALSE;

	return UpdateInternal();
}

BOOL CDib::ContrastStretching(float X1, float Y1, float X2, float Y2)
{
	if (IsEmpty())
		return FALSE;

	if (!ContrastStretchingDIB(m_hDib, X1, Y1, X2, Y2))
		return FALSE;

	return UpdateInternal();
}

BOOL CDib::LocalImageEnhancement()
{
	if (IsEmpty())
		return FALSE;

	if (!LocalImageEnhancementDIB(m_hDib))
		return FALSE;

	return UpdateInternal();
}

BOOL CDib::MyMedianFilter()
{
	if (IsEmpty())
		return FALSE;

	if (!MyMedianFilterDIB(m_hDib))
		return FALSE;

	return UpdateInternal();
}

BOOL CDib::SobleOperator()
{
	if (IsEmpty())
		return FALSE;

	if (!SobleOperatorDIB(m_hDib))
		return FALSE;

	return UpdateInternal();
}

BOOL CDib::LaplacianSharpening()
{
	if (IsEmpty())
		return FALSE;

	if (!LaplacianSharpeningDIB(m_hDib))
		return FALSE;

	return UpdateInternal();
}

BOOL CDib::PalmprintImageEnhancement()
{
	if (IsEmpty())
		return FALSE;

	if (!PalmprintImageEnhancementDIB(m_hDib))
		return FALSE;

	return UpdateInternal();
}

BOOL CDib::FrequencyDomainFilter()
{
	if (IsEmpty())
		return FALSE;

	if (!FrequencyDomainFilterDIB(m_hDib))
		return FALSE;

	return UpdateInternal();
}

BOOL CDib::HomomorphicFilter()
{
	if (IsEmpty())
		return FALSE;

	if (!HomomorphicFilterDIB(m_hDib))
		return FALSE;

	return UpdateInternal();
}

BOOL CDib::BattivoLowPassFilter()
{
	if (IsEmpty())
		return FALSE;

	if (!BattivoLowPassFilterDIB(m_hDib))
		return FALSE;

	return UpdateInternal();
}

BOOL CDib::BattivoHighPassFilter()
{
	if (IsEmpty())
		return FALSE;

	if (!BattivoHighPassFilterDIB(m_hDib))
		return FALSE;

	return UpdateInternal();
}

BOOL CDib::BattivoBandpassFilter()
{
	if (IsEmpty())
		return FALSE;

	if (!BattivoBandpassFilterDIB(m_hDib))
		return FALSE;

	return UpdateInternal();
}

BOOL CDib::BattivoBandRejectionFilter()
{
	if (IsEmpty())
		return FALSE;

	if (!BattivoBandRejectionFilterDIB(m_hDib))
		return FALSE;

	return UpdateInternal();
}

BOOL CDib::Exercise443()
{
	if (IsEmpty())
		return FALSE;

	if (!Exercise443DIB(m_hDib))
		return FALSE;

	return UpdateInternal();
}

BOOL CDib::VariableThreshold()
{
	if (IsEmpty())
		return FALSE;

	if (!VariableThresholdDIB(m_hDib))
		return FALSE;

	return UpdateInternal();
}

BOOL CDib::OstuImageSegmentation()
{
	if (IsEmpty())
		return FALSE;

	if (!OstuImageSegmentationDIB(m_hDib))
		return FALSE;

	return UpdateInternal();
}

BOOL CDib::FaceRecognition()
{
	if (IsEmpty())
		return FALSE;

	if (!FaceRecognitionDIB(m_hDib))
		return FALSE;

	return UpdateInternal();
}

BOOL CDib::IrisSegmentation()
{
	if (IsEmpty())
		return FALSE;

	if (!IrisSegmentationDIB(m_hDib))
		return FALSE;

	return UpdateInternal();
}

BOOL CDib::BoundaryTrackingAlgorithm()
{
	if (IsEmpty())
		return FALSE;

	if (!BoundaryTrackingAlgorithmDIB(m_hDib))
		return FALSE;

	return UpdateInternal();
}

BOOL CDib::RegionRefinement()
{
	if (IsEmpty())
		return FALSE;

	if (!RegionRefinementDIB(m_hDib))
		return FALSE;

	return UpdateInternal();
}

BOOL CDib::GraylevelCooccurrenceMatrix()
{
	if (IsEmpty())
		return FALSE;

	if (!GraylevelCooccurrenceMatrixDIB(m_hDib))
		return FALSE;

	return UpdateInternal();
}

BOOL CDib::Example1116()
{
	if (IsEmpty())
		return FALSE;

	if (!Example1116DIB(m_hDib))
		return FALSE;

	return UpdateInternal();
}

BOOL CDib::Example1127()
{
	if (IsEmpty())
		return FALSE;

	if (!Example1127DIB(m_hDib))
		return FALSE;

	return UpdateInternal();
}

BOOL CDib::FigureRecognition()
{
	if (IsEmpty())
		return FALSE;

	if (!FigureRecognitionDIB(m_hDib))
		return FALSE;
...
BOOL CDib::PeachImageRecognition()
{
	if (IsEmpty())
		return FALSE;

	if (!PeachImageRecognitionDIB(m_hDib))
		return FALSE;
